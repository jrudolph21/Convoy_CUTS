...
def trajectory_filter_distance(segments_a: Sequence[TrajectorySegment],
    ...
    incorporating Lemma 2 (Subset MBB pruning) to avoid exact distance calculations.
    """
    if not segments_a or not segments_b:
        return float('inf')

    # 1. Define Subset S (using segments_b) for Lemma 2
    b_min_x = min(min(s['x0'], s['x1']) for s in segments_b)
    b_max_x = max(max(s['x0'], s['x1']) for s in segments_b)
    b_min_y = min(min(s['y0'], s['y1']) for s in segments_b)
    b_max_y = max(max(s['y0'], s['y1']) for s in segments_b)
    b_max_tol = max(s['tolerance'] for s in segments_b) # delta_max(S)
    
    best = float('inf')
    
    for a in segments_a:
        # 2. Lemma 2: Segment-to-Subset MBB Pruning
        # expand_subset = e + delta(l'q) + delta_max(S)
        expand_subset = epsilon + a['tolerance'] + b_max_tol
        
        a_min_x = min(a['x0'], a['x1'])
        a_max_x = max(a['x0'], a['x1'])
        
        # Check X-axis overlap with Subset S
        if a_max_x + expand_subset < b_min_x or a_min_x - expand_subset > b_max_x:
            continue
            
        a_min_y = min(a['y0'], a['y1'])
        a_max_y = max(a['y0'], a['y1'])
        
        # Check Y-axis overlap with Subset S
        if a_max_y + expand_subset < b_min_y or a_min_y - expand_subset > b_max_y:
            continue

        # 3. Subset pruning failed; evaluate against individual segments in S
        for b in segments_b:
            # Strict temporal overlap check
            if max(a['t0'], b['t0']) > min(a['t1'], b['t1']):
                continue
                
            # Individual segment MBB check (effectively Lemma 2 where |S|=1)
            expand = epsilon + a['tolerance'] + b['tolerance']
            
            b_ind_min_x, b_ind_max_x = min(b['x0'], b['x1']), max(b['x0'], b['x1'])
            if a_max_x + expand < b_ind_min_x or a_min_x - expand > b_ind_max_x:
                continue
                
            b_ind_min_y, b_ind_max_y = min(b['y0'], b['y1']), max(b['y0'], b['y1'])
            if a_max_y + expand < b_ind_min_y or a_min_y - expand > b_ind_max_y:
                continue
            
            # Exact Geometry (Lemma 1) - Only runs if individual MBBs overlap
    ...
    try:
        ...
        if convoys:
            viz.visualize_convoys(
                ...
                start_time,
                end_time,
                file=f"all_convoys_map_beijing_cuts_class_v3{total_obj}_{total_time}_{total_dist}"
            )
