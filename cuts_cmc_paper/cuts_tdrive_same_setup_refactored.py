...
def cuts_filter(
    ...
    delta: float,
    lambda_points: int,
) -> Tuple[List[dict], Dict[int, List[dict]], Dict[int, pd.DataFrame], List[dict]]:
    """
...
if __name__ == "__main__":
    ...
    df_temp = df_temp.sort_values(['id', 'time']).drop_duplicates(['id', 'time']).reset_index(drop=True)

    print(f"Total rows in time window: {len(df_temp)}")
    print(f"Total unique taxis in window: {df_temp['id'].nunique()}")

    # Convert longitude/latitude to local meter coordinates for CuTS.
    # Keep df_temp unchanged so visualization still uses geographic coordinates.
    ...
    for i, c in enumerate(convoys):
        print(
            f"Convoy {i+1}: Objects {c['objects']} | "
